...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		List<NodeInterface> allNodes = node.getFingerTable();
		Node keyNode = null;
		for(NodeInterface n:allNodes) {
			if(n.getNodeID().compareTo(key) == 0) {
				keyNode = (Node)n;
			}
		}
		// get the successor of the node
		Node successorNode = (Node) keyNode.getSuccessor();
		// get the stub for this successor (Util.getProcessStub())
		successorNode = (Node) Util.getProcessStub(successorNode.getNodeName(), successorNode.getPort());
		// check that key is a member of the set {nodeid+1,...,succID} i.e. (nodeid+1 <= key <= succID) using the ComputeLogic
		boolean bo = Util.computeLogic(key, node.getNodeID(), successorNode.getNodeID());
		// if logic returns true, then return the successor
		if(bo) {
			return successorNode;
		}else {
			// if logic returns false; call findHighestPredecessor(key)
			Node highest_pred = (Node)findHighestPredecessor(key);
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		List<NodeInterface> enteries = node.getFingerTable();
		// starting from the last entry, iterate over the finger table
		Collection.reverse(enteries);
		for(NodeInterface i:enteries) {
			// for each finger, obtain a stub from the registry
			NodeInterface stub = Util.getProcessStub(i.getNodeName(), i.getPort());
			// check that finger is a member of the set {nodeID+1,...,ID-1} i.e. (nodeID+1 <= finger <= key-1) using the ComputeLogic
			boolean bo = Util.computeLogic(key, stub.getNodeID()+1, stub.getNodeID()-1);
