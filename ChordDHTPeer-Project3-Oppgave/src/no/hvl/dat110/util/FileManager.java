...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		for(int i = 0 ; i<numReplicas ; i++) {
			...
			// hash the replica
			BigInteger hashOfReplica = Hash.hashOf(replica);
			// store the hash in the replicafiles array.
			replicafiles[i] = hashOfReplica;
		}
    ...
    public int distributeReplicastoPeers() throws RemoteException {
    	...
    	for(BigInteger replica:replicafiles) {
        	...
        	NodeInterface successor = NodeInterface.findSuccessor(replica);
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for(BigInteger replica:replicafiles) {
			...
			NodeInterface s = NodeInterface.findSuccessor(replica);
			// get the metadata (Message) of the replica from the successor, s (i.e. active peer) of the file
			Message metadata = s.getFilesMetadata().get(replica);
	...
	public NodeInterface findPrimaryOfItem() {
		...
		Set<Message> n = activeNodesforFile;
		...
		for(Message m : n) {
			...
			boolean primary = m.isPrimaryServer();
			// return the primary
			if(primary) {
				return m.getNameOfFile();
			}
		}
